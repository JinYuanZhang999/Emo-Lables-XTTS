...
#BaseTrainerModel 继承自 TrainerModel，扩展了 🐸TTS 所需的功能。每个新的 🐸TTS 模型都必须继承它。
class BaseTrainerModel(TrainerModel):
    ...
    def inference(self, input: torch.Tensor, aux_input={}) -> Dict:
        #进行推理的前向传播。
        #它必须返回一个包含主要模型输出和所有辅助输出的字典。键 ```model_outputs``` 被认为是主要输出，您可以根据需要添加任何其他辅助输出。
        #我们不使用 `*kwargs`，因为它与 TorchScript API 兼容性较差。

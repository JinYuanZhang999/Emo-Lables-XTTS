...
class GPT(nn.Module):
    ...
    def get_logits(
        ...
        # Always compute latent representations
        text_latent = enc[:, : first_inputs.shape[1]]
        mel_latent = enc[:, -second_inputs.shape[1]:]

        if return_latent:
            return text_latent, mel_latent

        #return_latent=false时执行下面代码
        # 这里的first_logits和second_logits，其实就是text_logits和mel_logits
        ...
        if second_inputs is not None:
            ...
            return first_logits, second_logits,mel_latent
        else:#实际上并不会走这个分支
            return first_logits
    ...
    def forward(
        ...
        if cond_latents is None:
            ...
            # print("cond_latents 形状:", cond_latents.shape)
        ...
        # )
        if return_latent:   #这里返回的就是输入给HiFiGAN模块的latent
            text_latent,mel_latent = self.get_logits(
                text_emb,
                self.text_head,
                mel_emb,
                self.mel_head,
            ...
            return mel_latent[:, :sub]  # sub to prevent bla.
        else:
            text_logits, mel_logits,mel_latent = self.get_logits(
                text_emb,
                self.text_head,
                mel_emb,
                self.mel_head,
            ...
            mel_latent = mel_latent[:, :sub]
            # print(f"GPT.forward方法中的mel_latent:{mel_latent}")
            ...
            return loss_text.mean(), loss_mel.mean(), mel_logits,mel_latent

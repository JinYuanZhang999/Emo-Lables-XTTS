...
class VoiceBpeTokenizer:
    ...
    def preprocess_text(self, txt, lang):
        if lang in {"ar", "cs", "de", "en", "es", "fr", "hu", "it", "nl", "pl", "pt", "ru", "tr", "zh", "ko"}:
            txt = multilingual_cleaners(txt, lang) #去除标签中的尖括号<>
            # print(f"清理后的text：{txt}")
            if lang == "zh":
                txt = chinese_transliterate(txt)
                # print(f"中文翻译后的text：{txt}") #把汉字翻译成带音调的拼音，轻声标注为声调‘5’
            if lang == "ko":
    ...
    def encode(self, txt, lang):
        ...
        #从text中把标签取出来，防止在执行preprocess_text时对标签进行处理
        import re
        tag = ""  # 默认没有标签
        tag_match = re.match(r"(<[^<>]+>)(.*)", txt)
        if tag_match:
            tag = tag_match.group(1)      # '<ANGRY>'、'<HAPPY>'
            content = tag_match.group(2)  # '冬天雨非常多我不喜欢雨天'
            txt = content

        txt = self.preprocess_text(txt, lang) 
        # print(f"预处理后的text：{txt}")

        #把取出来的标签重新放到text中
        txt = tag + txt

        lang = "zh-cn" if lang == "zh" else lang
        txt = f"[{lang}]{txt}" #把语言标签放到文本前
        # print(f"加入语言标签后的text：{txt}")
        txt = txt.replace(" ", "[SPACE]")
        # print(f"替换空格后的text：{txt}") #中文没有空格需要替换
        return self.tokenizer.encode(txt).ids #对照vocab.json中的分词序号得到所有分词的需要

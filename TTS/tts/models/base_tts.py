...
#基础的 TTS 类。每个新的 TTS 模型都必须继承这个类
#它在 `Model` 实现的基础上定义了一些特定于 TTS 的通用功能
class BaseTTS(BaseTrainerModel):
    ...
    #初始化 BaseTTS 类，包含配置、音频处理器、分词器、说话人管理器和语言管理器
    def __init__(
        self,
        config: Coqpit,
        ap: "AudioProcessor",
    ...
    #根据配置类型设置模型参数 (`ModelConfig` 或 `ModelArgs`)
    #如果配置用于训练，名称中带有 "*Config"，则模型参数嵌入在 config.model_args 中
    #如果配置用于模型，名称中带有 "*Args"，则直接分配
    ...
    '''
    该实现有三种可能的结果：
        1. 如果 `config.use_speaker_embedding` 和 `config.use_d_vector_file` 都为 False，则不做任何操作。
        2. 如果 `config.use_d_vector_file` 为 True，则将嵌入通道大小设置为 `config.d_vector_dim` 或 512。
        3. 如果 `config.use_speaker_embedding`，则初始化一个说话人嵌入层，通道大小为 `config.d_vector_dim` 或 512。
    '''
    ...
    def get_sampler(self, config: Coqpit, dataset: TTSDataset, num_gpus=1):
        #支持三种加权采样器：语言加权、说话人加权和长度加权。根据配置参数计算权重，并返回
        #适当的 `WeightedRandomSampler` 或 `DistributedSampler`。
        ...
        else:  # 如果已经定义了采样器，则使用该采样器和 DDP 采样器
            sampler = DistributedSamplerWrapper(sampler) if num_gpus > 1 else sampler

        return sampler

    #根据配置和样本返回一个适当的数据加载器
    #处理多说话人和多语言属性，并初始化数据集和采样器
    def get_data_loader(
        self,
        config: Coqpit,
        assets: Dict,
    ...
    #为测试运行获取辅助输入
    #如果配置中使用了 d_vector_file，则从嵌入中随机选择一个 d_vector
    ...
    #在训练开始时保存 speaker.pth 和 language_ids.json。并更新它们的路径
    #如果有说话人管理器或语言管理器，则将 ID 文件保存到输出路径，并更新配置中的文件路径
...
#TTS 端到端基础类（负责设置模型参数），继承自 BaseTTS
class BaseTTSE2E(BaseTTS):
    #设置模型参数
    #根据配置的类型，设置模型参数 `num_chars` 和 `args`
    def _set_model_args(self, config: Coqpit):
        self.config = config
        if "Config" in config.__class__.__name__:
            num_chars = (
